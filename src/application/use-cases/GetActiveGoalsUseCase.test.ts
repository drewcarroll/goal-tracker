...
import { GetActiveGoalsUseCase } from "./GetActiveGoalsUseCase";
...
describe("GetActiveGoalsUseCase", () => {
  it("returns only the caller's goals", async () => {
    const useCase = new GetActiveGoalsUseCase(
  ...
  it("excludes paused goals", async () => {
    ...
    paused.pause();
    const active = goal("g2", "user-1");

    const useCase = new GetActiveGoalsUseCase(new InMemoryGoalRepository([paused, active]));
