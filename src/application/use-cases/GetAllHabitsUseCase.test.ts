...
import { GetAllHabitsUseCase } from "./GetAllHabitsUseCase";
...
describe("GetAllHabitsUseCase", () => {
  it("includes paused habits, unlike GetActiveHabitsUseCase", async () => {
    const paused = habit("h1", "user-1");
    paused.pause();
    const useCase = new GetAllHabitsUseCase(new InMemoryHabitRepository([paused]));
  ...
  it("returns only the caller's habits", async () => {
    const useCase = new GetAllHabitsUseCase(
